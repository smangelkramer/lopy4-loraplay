...
# Pysense Board + Sensor Libs laden
...
# Pysense initialisieren
py = Pysense()
si = SI7006A20(py)
lt = LTR329ALS01(py)
li = LIS2HH12(py)

# Heartbeat LED deaktivieren
pycom.heartbeat(False)

# LoRa in LoRaWAN Modus initialieren
lora = LoRa(mode=LoRa.LORAWAN)

# OTAA Parameter fuer TTN
app_eui = binascii.unhexlify('')
app_key = binascii.unhexlify('')

# OTAA Parameter ausgeben (Serial)
print("DevEUI: %s" % (binascii.hexlify(lora.mac())))
print("AppEUI: %s" % (binascii.hexlify(app_eui)))
print("AppKey: %s" % (binascii.hexlify(app_key)))


# Join TTN via OTAA (Over the Air Activation)
lora.join(activation=LoRa.OTAA, auth=(app_eui, app_key), timeout=0)

# Schleife wartet bis Dev joined ist
